...
# 为了计算显著图，我们首先需要计算出网络对于当前图片在正确类别上的得分，并且计算出损失在图像每个像素点上的梯度，然后对于每个像素点
# 取个通道中最大的梯度绝对值作为该像素点处的值。
def compute_saliency_maps(X, y, model1):
    """
    ...
    with  tf.GradientTape() as g:
        # watch 确保某个tensor被tape追踪
        g.watch(img)
        # 求分数
        scores = model1(img)
        # tf.gather(params,indices,axis=0) 从params的axis维根据indices的参数值获取切片从params的axis维根据indices的参数值获取切片
        correct_scores = tf.gather(scores, tf.stack([tf.range(N), y]), axis=1)
    grad = g.gradient(correct_scores, img)
...
def make_fooling_image(X, target_y, model):
    """
    生成一个接近X的虚假图像，但是模型对它进行了分类作为target_y。
...
def class_visualization_update_step(X, model, target_y, l2_reg, learning_rate):
    ...
    with tf.GradientTape() as tape:
        ...
        grad = tape.gradient( target_score,img)
        img.assign_add(learning_rate / tf.norm(grad) * grad)
    X = img
    pass
